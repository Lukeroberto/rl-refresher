...
class ReplayMemory():
    ...
    def store_transition(self, obs, action_prob, r, ret):
        ...
        self.returns[indx] = np.array(ret).copy()
    ...
    def sample_batch(self, batch_size):
        ...
        rewards = torch.as_tensor(self.rewards[batch])
        returns = torch.as_tensor(self.returns[batch])

        return obs, action_probs, rewards, returns
...
def train_on_batch(network, optimizer, batch, step, writer):
    ...
    h = network.representation(torch.unsqueeze(obs, 1))

    pred_rs, _ = network.dyamics(h)

    policy_logits, pred_vals = network.prediction(h)

    value_loss = F.mse_loss(pred_vals, returns)
    reward_loss = F.mse_loss(pred_rs, rewards)
